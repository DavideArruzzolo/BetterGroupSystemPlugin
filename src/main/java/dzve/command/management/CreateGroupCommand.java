...
public class CreateGroupCommand extends AbstractPlayerCommand {
    ...
    @Nonnull
    private final OptionalArg<String> color = withOptionalArg("color",
            "The color of the group (hex format, e.g., #FF0000)", ArgTypes.STRING);
    @Nonnull
    private final OptionalArg<String> description = withOptionalArg("description", "The description of the group",
            ArgTypes.STRING);
    ...
    protected void execute(@NonNullDecl CommandContext commandContext, @NonNullDecl Store<EntityStore> store,
        ...
        dzve.utils.LogService.info("COMMAND",
                "Player " + playerRef.getUsername() + " executed /"
                        + GroupService.getConfig().getAllCommandsPrefix() + " create "
                        + groupName);
